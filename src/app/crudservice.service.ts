...
export class CRUDService {
  ...
  public getElementsByProperty<T>(collectionName: string, property: string, propName: string): Observable<T[]> {
  ...
  public updateObject(collectionName: string, id: string, obj: object): Observable<void> {
    return from(this.firestoreService.collection(collectionName).doc(id).set({...obj}, {merge: true}))
      .pipe(
        take(1));
  }

  public deleteObject(collectionName: string, id: string): Observable<void> {
    return from(this.firestoreService.collection(collectionName).doc(id).delete())
      .pipe(
        take(1));
  }
}
